...
public class Main {
	...
	private void solve() {
		...
		int dp1_length = Math.min(sum_t, T);
		long[] dp1 = new long[dp1_length + 1];
		for (int i = 0; i < N; i++) {
			for (int c_t = dp1_length - t[i]; c_t >= 0; c_t--) {
				dp1[c_t + t[i]] = Math.max(dp1[c_t + t[i]], dp1[c_t] + p[i]);
			}
		}
		long[] dp2 = new long[T + K + 1];
		for (int i = 0; i <= dp1_length; i++) {
			for (int c_t = 0; c_t + i <= T; c_t++) {
				dp2[c_t + i + K] = Math.max(dp2[c_t + i + K], dp2[c_t] + dp1[i]);
			}
		}
		long ans = 0L;
		for (int i = 0; i <= T + K; i++) {
